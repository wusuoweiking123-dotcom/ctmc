...
def simulate_cev(sigma, beta, T, N, f0=100):
    dt = T / N
    f = np.zeros(N)
    f[0] = f0
    np.random.seed(51)
...
def main():
    ...
    print("\n步骤2: 离散化状态空间...")
    num_bins = 800
    state_indices, bins = discretize_data(f, num_bins)
    centers = (bins[:-1] + bins[1:]) / 2
    ...
    european_put = np.max(K * np.exp(-r * T) * norm.cdf(-d2) - f0 * norm.cdf(-d1),0)
    european_call = np.max(f0 * norm.cdf(d1) - K * np.exp(-r * T) * norm.cdf(d2),0)
    ...
    for center in centers[::3]:  # 显示部分状态中心点
        plt.axvline(x=center, color='orange', alpha=0.3, linewidth=0.5)
    plt.xlabel('Price')
    plt.ylabel('Density')
    plt.title('State Space Discretization')
    plt.legend()
    plt.grid(True)

    # 准备时间网格
    time_points = np.arange(0, M + 1) * dt

    # time_indices = np.linspace(0, len(f)-1, M+1, dtype=int)
    # f_resampled = f[time_indices]
    ...
    for t in range(M + 1):
        ...
        # 在每个时间点，根据当前价格插值计算期权价值
        call_value =  get_option_price_mean(V_call[t, :])
        put_value =  get_option_price_mean(V_put[t, :])

        call_values_path.append(call_value)
        put_values_path.append(put_value)

    call_values_path = np.array(call_values_path)
    put_values_path = np.array(put_values_path)

    # 子图3：美式看涨期权价值随时间变化（沿着价格路径）
    plt.subplot(2, 2, 3)
    plt.plot(time_points, call_values_path, 'b-', linewidth=2, marker='o', markersize=3)
    plt.xlabel('Time')
    plt.ylabel('Call Option Value')
    plt.title('American Call Option Value Along Price Path')
    plt.grid(True)

    # 添加关键信息
    plt.text(0.02, 0.95, f'Initial Value: {call_values_path[0]:.4f}',
             transform=plt.gca().transAxes, verticalalignment='top',
             bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.8))

    # 子图4：美式看跌期权价值随时间变化（沿着价格路径）
    plt.subplot(2, 2, 4)
    plt.plot(time_points, put_values_path, 'r-', linewidth=2, marker='o', markersize=3)
    plt.xlabel('Time')
    plt.ylabel('Put Option Value')
    plt.title('American Put Option Value Along Price Path')
    plt.grid(True)

    # 添加关键信息
    plt.text(0.02, 0.95, f'Initial Value: {put_values_path[0]:.4f}',
             transform=plt.gca().transAxes, verticalalignment='top',
