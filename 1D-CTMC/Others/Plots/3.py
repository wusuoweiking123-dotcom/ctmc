...
def nadaraya_watson_estimation(data, tau, date, h1, h2):
    numerator = np.sum(data['Implied_volatility'] * gaussian_kernel(tau - data['tau'], date - date2num(data['Date']), h1, h2))
    denominator = np.sum(gaussian_kernel(tau - data['tau'], date - date2num(data['Date']), h1, h2))
    return numerator / denominator

# Construct smooth surface
def smooth_surface(data, grid_size=50, h1=0.05, h2=0.05):
    m_range = np.linspace(data['tau'].min(), data['tau'].max(), grid_size)
...
ax.set_zlabel('Smooth Implied Volatility',fontsize=14)
